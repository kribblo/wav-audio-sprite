...
function wavAudioSprite(files, callback) {
    ...
    files.forEach((file) => {
        ...
        if(audio.channelData.length !== channels) {
            throw `Different number of channels in wav file: ${file} has ${audio.channelData.length}, previous had ${channels}`;
        }

        if(audio.sampleRate !== sampleRate) {
            throw `Different sample rate in wav file: ${file} has ${audio.sampleRate}, previous had ${sampleRate}`;
        }
