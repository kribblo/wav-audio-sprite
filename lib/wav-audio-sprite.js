...
function wavAudioSprite(files) {
    ...
    files.forEach((file) => {
        ...
        if(audio.channelData.length !== channels) {
            return Promise.reject(`Different number of channels in wav file: ${file} has ${audio.channelData.length}, previous had ${channels}`);
        }

        if(audio.sampleRate !== sampleRate) {
            return Promise.reject(`Different sample rate in wav file: ${file} has ${audio.sampleRate}, previous had ${sampleRate}`);
        }
    ...
    return Promise.resolve(encoded, timings);
}
